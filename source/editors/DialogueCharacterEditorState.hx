...
class DialogueCharacterEditorState extends MusicBeatState
{
	...
	override function create() {
		...
		tipText.setFormat(Paths.font("fullphanmuff.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		tipText.cameras = [camOther];
		tipText.scrollFactor.set();
		add(tipText);

		offsetLoopText = new FlxText(10, 10, 0, '', 32);
		offsetLoopText.setFormat(Paths.font("fullphanmuff.ttf"), 32, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		offsetIdleText.setFormat(Paths.font("fullphanmuff.ttf"), 32, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	private static var DEFAULT_TEXT:String = 'AUTO TEXT';
	function reloadText() {
		if(daText != null) {
			daText.killTheTimer();
			daText.kill();
